...
const removeDomain = (item: DomainItem) => {
  const index = dynamicValidateForm.subcategories.indexOf(item)
  if (index !== -1) {
    dynamicValidateForm.subcategories.splice(index, 1)
  }
}

const addDomain = () => {
  dynamicValidateForm.subcategories.push({
    id: randomString(32),
    mainId: mainId,
    name: ''
  })
...
const save = async () => {
  categoriesStore.addCategoriesData(dynamicValidateForm)
}
